...
fn count_possible_paths(caves_input: Vec<String>) -> i32 {
    ...
    follow_and_count_paths(&caves, &Vec::new(), START_TOKEN.to_string(), false)
}
...
fn follow_and_count_paths(
    ...
    single_cave_visited_twice: bool,
) -> i32 {
    ...
    if visited_caves.contains(&current_cave.name)
        && single_cave_visited_twice
        && matches!(current_cave.cave_type, CaveType::SMALL)
    {
        return 0;
    }

    if visited_caves.last() == Some(&current_cave.name) {
        return 0;
    }

    let current_cave_visited_count = visited_caves
        .iter()
        .filter(|visited_cave| current_cave.name == **visited_cave)
        .collect::<Vec<&String>>()
        .len();

    if current_cave_visited_count == 2 && matches!(current_cave.cave_type, CaveType::SMALL) {
        return 0;
    }

    let mut new_visited_caves = visited_caves.clone();
    new_visited_caves.push(current_cave.name.clone());

    let current_cave_is_small_and_visited_twice =
        current_cave_visited_count == 1 && matches!(current_cave.cave_type, CaveType::SMALL);
    ...
    current_cave
        ...
        .fold(0, |path_count, linked_cave| {
            path_count
                + follow_and_count_paths(
                    &caves,
                    &new_visited_caves,
                    linked_cave.clone(),
                    single_cave_visited_twice || current_cave_is_small_and_visited_twice,
                )
        })
}
...
mod tests {
    ...
    fn should_count_possible_paths_for_example_input() {
        ...
        assert_eq!(count_possible_paths(caves_input), 36);
    }
    ...
    fn should_count_possible_paths_for_exercise_input() {
        ...
        assert_eq!(count_possible_paths(caves_input), 147784);
    }
}
