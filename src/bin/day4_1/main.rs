...
fn play_bingo(bingo_input: Vec<String>) -> i32 {
    ...
    for bingo_number in bingo_numbers {
        check_card_numbers(bingo_number, &mut bingo_cards);

        for bingo_card in &bingo_cards {
            if is_a_winner_card(bingo_card) {
                return calculate_winner_card_result(bingo_number, bingo_card);
            }
        }
    }

    panic!("Winner Card not found")
}
...
fn create_bingo_cards(bingo_input: &Vec<String>) -> Vec<BingoCard> {
    let mut bingo_card = BingoCard { lines: Vec::new() };
    ...
    for i in 2..bingo_input.len() {
        ...
        if line.len() == 0 {
            ...
            bingo_card = BingoCard { lines: Vec::new() };
        } else {
...
fn check_card_numbers(bingo_number: &str, bingo_cards: &mut Vec<BingoCard>) {
    for bingo_card in bingo_cards {
...
mod tests {
    ...
    #[test]
    fn should_find_the_first_winning_bingo_card_with_small_input() {
        let bingo_input = input::lines_from_file(SMALL_INPUT_FILE)
            .expect("Something went wrong reading the file");

        assert_eq!(play_bingo(bingo_input), 4512);
    }
    ...
    fn should_find_the_first_winning_bingo_card_with_large_input() {
        ...
        assert_eq!(play_bingo(bingo_input), 46920);
    }
}
