...
fn explore_cave_to_get_risk_from_point(
    ...
    let max_difference = max(start_x - x, start_y - y);

    if max_difference > 0 && cave_risk_map[y as usize][x as usize] + max_difference > MAX_RISK - 2 {
        return i32::MAX;
    }
...
mod tests {
    ...
    fn should_calculate_minimum_risk_for_small_input() {
        ...
        assert_eq!(find_path_with_minimum_risk(chiton_caves_input), 40);
    }
    ...
    fn should_calculate_minimum_risk_for_large_input() {
        ...
        assert_eq!(find_path_with_minimum_risk(chiton_caves_input), 755);
    }
}
