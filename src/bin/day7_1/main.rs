...
fn main() {
    let crabs_positions_input = input::lines_from_file(LARGE_INPUT_FILE).expect("Could not load lines");
...
fn calculate_minimum_spent_fuel(lanternfish_input: Vec<String>) -> i32 {
    ...
    let max_position = crabs_positions.iter().fold(0, |max_position, position| max(max_position, *position));

    let mut minimum_fuel = None;

    for i in 0..=max_position {
        let spent_fuel = crabs_positions.iter().fold(0, |spent_fuel, position| spent_fuel + i32::abs(position - i));
...
fn create_crabs_positions(crabs_positions_input: &Vec<String>) -> Vec<i32> {
    crabs_positions_input[0].split(",").map(|state| state.parse::<i32>().unwrap()).collect()
}
...
mod tests {
    ...
    fn should_calculate_minimum_spent_fuel_for_small_input() {
        ...
        assert_eq!(calculate_minimum_spent_fuel(crabs_positions_input), 37);
    }
    ...
    fn should_calculate_minimum_spent_fuel_for_large_input() {
        ...
        assert_eq!(calculate_minimum_spent_fuel(crabs_positions_input), 348996);
    }
}