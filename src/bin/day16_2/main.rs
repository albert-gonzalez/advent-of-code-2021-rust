...
const SUM_TYPE_ID: usize = 0;
const PRODUCT_TYPE_ID: usize = 1;
const MIN_TYPE_ID: usize = 2;
const MAX_TYPE_ID: usize = 3;
const LITERAL_TYPE_ID: usize = 4;
const GT_TYPE_ID: usize = 5;
const LT_TYPE_ID: usize = 6;
const EQ_TYPE_ID: usize = 7;
...
fn main() {
    ...
    let result = run_packets_operators(bits_hex_input);

    println!("The result is: {}", result);
}

fn run_packets_operators(bits_hex_input: Vec<String>) -> usize {
...
fn run(packet: &Packet) -> usize {
    match packet.type_id {
        SUM_TYPE_ID => sum(&packet.sub_packets),

        PRODUCT_TYPE_ID => product(&packet.sub_packets),

        MIN_TYPE_ID => minimum(&packet.sub_packets),

        MAX_TYPE_ID => maximum(&packet.sub_packets),

        GT_TYPE_ID => gt(&packet.sub_packets),

        LT_TYPE_ID => lt(&packet.sub_packets),

        EQ_TYPE_ID => eq(&packet.sub_packets),

        LITERAL_TYPE_ID => packet.literal,
        _ => 0,
    }
}

fn sum(packets: &Vec<Packet>) -> usize {
    packets.iter().fold(0, |sum, packet| sum + run(packet))
}

fn product(packets: &Vec<Packet>) -> usize {
    packets.iter().fold(1, |sum, packet| sum * run(packet))
}

fn maximum(packets: &Vec<Packet>) -> usize {
    packets
        .iter()
        .fold(usize::MIN, |max_num, packet| max(max_num, run(packet)))
}

fn minimum(packets: &Vec<Packet>) -> usize {
    packets
        .iter()
        .fold(usize::MAX, |min_num, packet| min(min_num, run(packet)))
}

fn lt(packets: &Vec<Packet>) -> usize {
    if run(&packets[0]) < run(&packets[1]) {
        1
    } else {
        0
    }
...
fn gt(packets: &Vec<Packet>) -> usize {
    if run(&packets[0]) > run(&packets[1]) {
        1
    } else {
        0
...
fn eq(packets: &Vec<Packet>) -> usize {
    if run(&packets[0]) == run(&packets[1]) {
        1
    } else {
        0
...
fn parse_literal_packet(binary_input: &String, mut current_index: usize) -> (usize, usize) {
    ...
    (
        usize::from_str_radix(&literal_in_bits, 2).unwrap(),
        current_index,
    )
}
...
mod tests {
    ...
    #[test]
    fn should_run_operators_and_return_the_result_for_exercise_input() {
        let hex_input = input::lines_from_file(EXERCISE_INPUT_FILE)
            .expect("Something went wrong reading the file");

        assert_eq!(run_packets_operators(hex_input), 9485076995911);
    }
}
