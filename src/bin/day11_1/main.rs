...
fn main() {
    ...
    let flashes_count = calculate_octopus_flashes_after_100_steps(dumbo_octopuses_input);

    println!("The result is: {}", flashes_count);
}

fn calculate_octopus_flashes_after_100_steps(dumbo_octopuses_input: Vec<String>) -> i32 {
    let mut energy_map = create_energy_map(&dumbo_octopuses_input);

    let mut flash_count = 0;
    for _i in 0..100 {
        flash_count += calculate_flashes_in_step(&mut energy_map);
    }
    flash_count
}
...
fn calculate_flashes_in_step(energy_map: &mut Vec<Vec<i32>>) -> i32 {
...
mod tests {
    ...
    fn should_calculate_octopus_flashes_after_100_steps_for_example_input() {
        ...
        assert_eq!(
            calculate_octopus_flashes_after_100_steps(dumbo_octopuses_input),
            1656
        );
    }
